"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.handleCredentialsAtLogin = void 0;
const jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
const login_model_1 = require("../models/login.model");
const codes_utils_1 = require("../utils/codes.utils");
const handleCredentialsAtLogin = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { email, password } = req.body;
        const user = yield (0, login_model_1.verifyCredentials)(email, password);
        const token = jsonwebtoken_1.default.sign({ id: user.id, email: user.email }, process.env.JWT_SECRET, { expiresIn: "1h" });
        const successResponse = (0, codes_utils_1.handleSuccess)(200);
        res.status(successResponse.status).json({
            message: successResponse.message,
            token,
            user: {
                email: user.email,
                id: user.id,
            },
        });
    }
    catch (error) {
        if (error instanceof Error) {
            console.error("Error during credentials verification:", error.message);
        }
        else {
            console.error("Unexpected error:", error);
        }
        const errorResponse = (0, codes_utils_1.handleErrors)(401);
        res.status(errorResponse.status).json({ error: errorResponse.message });
    }
});
exports.handleCredentialsAtLogin = handleCredentialsAtLogin;
